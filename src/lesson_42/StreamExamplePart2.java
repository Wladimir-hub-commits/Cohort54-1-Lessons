...
public class StreamExamplePart2 {
   ...
   10. distinct() - Удаляет дубликаты из потока. Сравнивает (определяет равенство) методом equals()
...
//        System.out.println(findMe.getName());
    ...
    // удаляет дубликаты (повторяющиеся значения)
    private static void task10() {
        // Удалить дубликаты (повторяющиеся значения) из списка
        List<Integer> integers = Arrays.asList(10, 20, 30, 20, 30, 50, 0);// есть дубликаты
        ...
        // метод equals() сравнивает объекты по значению полей
        // метод distinct() удаляет дубликаты
        // метод distinct() сравнивает объекты на равенство с использованием метода equals()
    ...
    private static void task9() {
        ...
        map.put("Apple", 1);// добавление значения
        map.put("Banana", -10);// добавление значения (-10 как Value)
        map.put("Cherry", 5);// добавление значения

        // Создание стрима из элементов entrySet
        // на map у меня нет возможности вызвать entrySet()
        map.entrySet().stream() // коллекция Map.EntrySet - вхождение в пара - ключ и значение
                .filter(entry -> entry.getValue() > 0)// проверка вхождения больше нуля.
                // При фильтрации entrySet не происходит выброса из map каких либо значений
                .forEach(entry -> System.out.println(entry.getKey() + ": " + entry.getValue()));// вывод ключа и значения
